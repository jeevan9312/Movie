...
# ── Load model (auto-trains on first run) ──────────────────────────────────────
@st.cache_resource(show_spinner="Setting up recommendation engine...")
def load_model():
    from models.recommender import HybridRecommender
    from utils.data_loader import load_all

    if not HybridRecommender.exists():
...
with st.sidebar:
    ...
    page = st.radio("📌 Navigate", ["🔍 Recommendations", "🔬 Algorithm Info"])
...
with st.spinner("🚀 Loading model (first visit trains automatically ~60s)..."):
    model = load_model()
...
if "Recommendations" in page:
    ...
    with col1:
        ...
        selected_movie = st.selectbox("🎥 Search for a movie", options=all_titles, index=default_idx)
    with col2:
...
# PAGE: Algorithm Info
# ─────────────────────────────────────────────────────────────────────────────
elif "Algorithm" in page:
    st.markdown('<div class="section-header">🔬 Algorithm Breakdown</div>', unsafe_allow_html=True)

    all_titles = sorted(movies_df["title"].tolist())
    default_idx = all_titles.index("Toy Story (1995)") if "Toy Story (1995)" in all_titles else 0
    selected_movie = st.selectbox("Pick a movie to analyse", options=all_titles, index=default_idx)
